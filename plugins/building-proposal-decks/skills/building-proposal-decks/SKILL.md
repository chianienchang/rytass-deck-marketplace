...
- **一次性設定**（建後端 + 託管）：照 `comment-backend/SETUP.md` 做（Google 試算表 + Apps Script 取得 endpoint；Cloudflare Pages 上傳取得審稿網址）。所有簡報共用同一後端。
- **每份簡報**：在 `<script>` 裡把 `EZC.endpoint` 填共用 endpoint、`EZC.deckId` 填這份的唯一代號（如 `ezmoney-v1`）。建議為每張 `.slide` 加穩定 `data-sid="..."`（不填則自動用頁序，但增刪頁後對應會位移）。
- **審稿分享**：把整包上傳 Cloudflare Pages，網址給同事；**審稿網址勿給客戶**。
- **元件圖釘留言（像 Figma）**：在審稿網址上開「💬 註解模式」後，點「📌 釘選元件」再點展示頁網頁中的某元件（如按鈕）即可釘留言；點圖釘看討論、resolve 後消失。**僅在 Cloudflare 同源網址可用**，本機開檔會自動停用並提示。後端若是舊版，需在 Apps Script「管理部署作業 → 編輯 → 新版本」重新部署，才有 `anchor` 欄位。
- **產出客戶版（移除留言功能）**：
  ```bash
...
- `template.html` — 固定外殼起始檔（新版 CSS + JS + 空 body，含 COMMENTS 註解層）
